...
現行実装は単一種の再現ではなく、アルゼンチンアリ (`Linepithema humile`) の局所的なフェロモン追従と、クロヤマアリ属の一種 (`Lasius niger`) で報告された経路記憶・沈着調整を組み合わせた混合簡略モデルです。
...
| 経路記憶とフェロモンの併用 | `Lasius niger` の経路学習研究では記憶とフェロモンの相互作用が調べられている。ただし Czaczkes et al. 2016 は、実験条件下でフェロモンが経路学習を支えた証拠を得られなかった。 | `memoryHeading` とフェロモン追従を別の弱い入力として加算 | 混合簡略モデル。フェロモンが学習を直接強化するとは説明しない |
| 障害物後の行列再安定 | 実際の採餌では、局所探索、既存の経路記憶、成功した経路への追従が合わさり、長時間では通れる経路へ行列が再形成される。 | 置かれた障害物を経路生成に反映し、進捗が落ちた個体は再合流ターンを強める | 追加 |
...
| 混雑時の自己調整 | Czaczkes et al. 2013 は、`Lasius niger` で混雑が pheromone deposition を下げる負のフィードバックになり得ると報告。 | 近傍個体数が多いほど沈着量を下げる | 追加。密度と倍率はゲーム向け簡略値 |
| フェロモンと歩行速度 | Perna et al. 2012 のアルゼンチンアリでは、短時間の歩行速度は局所フェロモン濃度に大きく左右されなかった。 | 個体ごとの基準速度を持ち、フェロモン量から速度を直接加速しない | 追加 |
...
- 障害物迂回は、本物の地形学習を厳密に再現するものではなく、「通れない場所の周りに短い迂回候補ができ、時間経過で個体がそこへ戻る」ゲーム向けの抽象化。
...
1. 経路記憶を視覚ランドマークや path integration として分離する。
   - 現状の `memoryHeading` は成功経路へのゲーム的な慣性であり、実際の記憶機構を再現していない。
